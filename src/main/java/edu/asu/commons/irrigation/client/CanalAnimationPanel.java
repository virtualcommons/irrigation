...
*/}
	...
	protected void paintComponent(Graphics graphics){
		...
		//System.out.println("Gate :"+5+" X: "+gate[5].getX()+" Y :"+gate[5].getY());
		...
		for(int i=0;i<BALLCOUNT;i++){
			/*if(!(balls[i].isInServer == false && clientGameState.getGroupDataModel().getMaximumAvailableBandwidth() == 0)){
				if(!(flagDownStream == true && flagUpStream == true && balls[i].x >= 950))	
			if(!(flagUpStream == true && clientGameState.getGroupDataModel().getMaximumAvailableBandwidth() <=25 && balls[i].x >=420)){
				if(flagUpStream == true && balls[i].x >=420 && clientGameState.getGroupDataModel().getMaximumAvailableBandwidth() >=25 ){
			...
			*/
			//System.out.println("ball sposition x :"+balls[i].x+" y:"+balls[i].y);
	...
	private void process(int i){
		...
		case 0: if((balls[i].x >= (100 - 20) && balls[i].x <= 100) && (balls[i].y >= 100-(int)(totalContributedBandwidth*0.8) &&
				balls[i].y <= 100)){
