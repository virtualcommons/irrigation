...
public class ActivitySummaryPanelNew extends JPanel {
	...
	//create three Maps for every parameter
	...
	//private int NUMBER_OF_PARTICIPANTS_PERGROUP = 2;

	private int parameterIndex;
	
	IrrigationClient client;  //  @jve:decl-index=0:
	...
	private void initialize(int parameterIndex,IrrigationClient client) {
		...
		case 0 : //add the progress Bbar
					...
					for(int i=0;i<client.getRoundConfiguration().getClientsPerGroup();i++){
						...
						if(i == client.getClientGameState().getPriority()){
							...
							availableBandwidthProgressBar.setForeground(new Color(51, 153, 255));	
		...
		case 1 : //add the number of files downloaded
					...
					for(int i=0;i<client.getRoundConfiguration().getClientsPerGroup();i++){
						...
						filesDownLoaded.setForeground(new Color(51,153,255));
		...
		case 2:  //add the number of tokens collected
					this.setBackground(Color.WHITE);
					...
					for(int i=0;i<client.getRoundConfiguration().getClientsPerGroup();i++){
						...
						tokensCollected.setForeground(new Color(51,153,255));
	...
	public void update(ClientData clientData) {
		...
		case 2: 
			tokensCollectedMap.get(new Integer(clientData.getPriority())).setText(new Integer(clientData.getTotalTokensEarned()).toString());
			break;
		}
	}
